document.addEventListener("DOMContentLoaded", () => {
  ...
  visualFixes.textContent = `
    ...
    .collection-leather, .collection-wood { min-height:590px !important; overflow:hidden !important; }
    .collection-leather .collection-overlay, .collection-wood .collection-overlay { z-index:1 !important; }
    .collection-leather .collection-content, .collection-wood .collection-content { z-index:5 !important; }
    ...
    @media (max-width:600px) {
      ...
      .collection-leather, .collection-wood { min-height:600px !important; }
    }
  `;
  ...
  if (menuToggle && navigation) {
    ...
    navigation.querySelectorAll("a").forEach((link) => link.addEventListener("click", () => {
      navigation.classList.remove("open"); menuToggle.classList.remove("active");
      menuToggle.setAttribute("aria-expanded", "false"); menuToggle.setAttribute("aria-label", "Open navigation");
    }));
  }

  const updateHeader = () => { if (header) header.style.boxShadow = window.scrollY > 20 ? "0 12px 35px rgba(23,19,15,.08)" : "none"; };
  ...
  document.querySelectorAll('a[href^="#"]').forEach((link) => link.addEventListener("click", (event) => {
    ...
    event.preventDefault(); const headerHeight = header ? header.offsetHeight : 0;
  ...
  const woodCard = document.querySelector(".collection-wood");
  ...
  if ("IntersectionObserver" in window) {
    const observer = new IntersectionObserver((entries, observerInstance) => entries.forEach((entry) => {
      if (!entry.isIntersecting) return; entry.target.classList.add("is-visible"); observerInstance.unobserve(entry.target);
